...
window.onscroll = function () {
  ...
  if (inst_scroll > hide_clust) {
    d3.select("#container-id-1 .viz_svg").style("display", "none");
  } else {
    d3.select("#container-id-1 .viz_svg").style("display", "block");
  }

  // hide col sim mat
  if (inst_scroll > hide_col_sim || inst_scroll < show_col_sim) {
    d3.select("#container-id-2 .viz_svg").style("display", "none");
  } else {
    d3.select("#container-id-2 .viz_svg").style("display", "block");
  }
};
...
function dendro_callback(inst_selection) {
  ...
  if (inst_data.inst_rc === "row") {
    if (clust_num !== "2") {
      d3.selectAll(".enrichr_export_section").style("display", "block");
    } else {
      d3.selectAll(".enrichr_export_section").style("display", "none");
    }
  } else {
    d3.selectAll(".enrichr_export_section").style("display", "none");
  }
...
function resize_container() {
  ...
  d3.selectAll(".clustergrammer_container").style(
    "width",
    container_width + "px"
  );
}
