...
function run_ini_enrichr(inst_cgm) {
  ...
  if (genes_were_found[inst_root] || make_enrichrgram) {
    if (d3.select(inst_root + " .enrichr_logo").empty()) {
      ...
      var enrichr_section = d3
        .selectAll(inst_root + " .dendro_info")
        .select(".modal-body")
        .append("div")
      ...
      enrichr_section
        ...
        .on("click", function () {
          var group_string = d3
            .select(inst_root + " .dendro_text input")
            .attr("value");
...
function Enrichrgram(inst_cgm) {
  ...
  function enrichr_icon() {
    ...
    var enr_logo = d3
      ...
      .attr(
        "xlink:href",
        "https://amp.pharm.mssm.edu/Enrichr/images/enrichr-icon.png"
      )
      .style("opacity", low_opacity)
      .classed("enrichr_logo", true)
      .attr("id", function () {
        var inst_id =
          "enrichr_menu_button_" + inst_cgm.params.root.replace("#", "");
        return inst_id;
      })
      .on("click", function () {
        toggle_enrichr_menu();
      ...
      .on("mouseover", function () {
        var is_showing = d3
          .select(inst_cgm.params.root + " .enrichr_menu")
          .classed("showing");
    ...
    var enr_menu = d3
      .select(inst_cgm.params.root + " .viz_svg")
      .append("g")
    ...
    enr_export_container
      ...
      .on("click", function () {
        ...
        var enrichr_info = {
          list: gene_list,
          description: "Clustergrammer gene list",
          popup: true,
        };
    ...
    lib_groups.on("click", function (d) {
  ...
  function clear_enrichr_results(run_resize_viz) {
    d3.select(inst_cgm.params.root + " .enr_menu_clear").style(
      "display",
      "none"
    );
  ...
  function toggle_enrichr_menu() {
    ...
    if (enr_menu.classed("showing") === false) {
      // show menu
      ///////////////////////
      enr_menu.classed("showing", true).style("display", "block");
      ...
      $.ajax(settings)
        ...
        .fail(function (jqXHR, textStatus, errorThrown) {
          ...
          if (enr_obj.get_tries < 2) {
            ...
            console.log(
              "did not get response from Enrichr - need to remove wait circle"
            );
            d3.select(inst_cgm.params.root + " .enr_wait_circle").remove();
          }
        });
  ...
  function update_viz_callback(enr_obj) {
    ...
    var enr_title = d3
      ...
      .attr("transform", function () {
        var trans = d3
          .select(inst_cgm.params.root + " .row_cat_label_container")
          .attr("transform")
          .split("(")[1]
          .split(")")[0];
