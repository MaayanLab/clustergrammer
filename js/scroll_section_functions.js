...
section_fun["run_filter_var_10"] = function () {
  ...
  d3.select(cgm.params.root + " .title_N_row_var").text(
    "Top rows variance: 10"
  );

  d3.select(cgm.params.root + " .slider_N_row_var").attr("current_state", "10");
};
...
function update_section(current_section) {
  if (prev_section != current_section) {
    ...
    if (
      d3.select(".toggle_col_order").select("button").attr("disabled") === null
    ) {
      inst_function();
...
function click_lib(lib_of_interest) {
  found_lib = d3
    .select(cgm.params.root + " .enr_lib_section")
    .selectAll("g")
    .filter(function () {
