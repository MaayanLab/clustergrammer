...
function downsample(params, min_rect_height){
  ...
  console.log('reduce_num_rows_by '+String(reduce_num_rows_by))
  console.log('ini_rect_height '+String(ini_rect_height))
  console.log('the min rect height is set to '+String(min_rect_height))

  var col_nodes = params.network_data.col_nodes;

  var new_num_rows = Math.floor(ini_num_rows/reduce_num_rows_by);

  console.log('ini num rows '+String(ini_num_rows));
  ...
  console.log('new tile height '+String(tile_height))
  ...
  console.log('starting number of links '+String(links_in_view.length))
  console.log('the number of links has been reduced to '+String(new_links.length))
  ...
  console.log('\n\nchecking data bound to clust_group rows ')

  // update bound data on rows 
  ds_clust = d3.select('#clust_group')
    .selectAll('.row')
    .data(matrix, function(d){return d.name;});
    ...
    var tile = d3.select(this)
      ...
      tile
        .style('fill-opacity', function(d){
          val_updn = Math.abs(d.value_up)+Math.abs(d.value_dn);
          output_opacity = params.matrix.opacity_scale(val_updn*ds_factor);
